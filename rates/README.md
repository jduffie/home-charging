...
```
RATE_USD_PER_KWH = 0.14   # PLACEHOLDER — replace with your blended home rate
```

- **How to find your number:** read it off a recent utility bill — total `$` ÷
  total `kWh`.
- **Referenced downstream:** downstream cost dashboards reference THIS value; they
  do not define their own. Change the constant above and downstream cost math
  follows.
- **Flat blended rate, on purpose:** a single number — no time-of-use (TOU) or
  ...
  (it ripples into the downstream cost dashboards too) — see the *TOU / tiered
